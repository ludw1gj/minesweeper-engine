import { createRandomNumberGenerator } from './random.ts'
import { Cell, Coordinate, Difficulty, Grid } from './types.ts'
...
export function revealCellInGrid(grid: Grid, atCoordinate: Coordinate): Grid {
  ...
  return newGrid.map((row) =>
    row.map((cell) =>
      adjacentCells.includes(cell) ? { ...cell, status: 'revealed' } : cell
    )
  )
}
...
export function revealAllCells(grid: Grid): Grid {
  return grid.map((row) =>
    row.map((cell) =>
      cell.status !== 'revealed' ? { ...cell, status: 'revealed' } : cell
    )
  )
}
...
export function initiateGrid(
  ...
  const newGrid = grid.map((row, y) =>
    row.map((cell, x): Cell => ({
      status: cell.status === 'flagged' ? 'flagged' : 'hidden',
      mineCount: mineCoordinates.some((mineCoordinate) =>
          areCoordinatesEqual(mineCoordinate, { x, y })
        )
        ? -1
        : countAdjacentMines(mineCoordinates, { x, y }),
    }))
  )
 ...
 * all cells.
 */
export function setLoseState(
  grid: Grid,
  detonationCoordinate: Coordinate,
): Grid {
  return grid.map((row, y) =>
    row.map((cell, x) =>
...
export function isWinGrid(grid: Grid): boolean {
  const { revealedWaterCells, mines, totalCells } = grid
    ...
    .reduce(
      (totalCount, cell) => ({
        revealedWaterCells: cell.status === 'revealed' && cell.mineCount !== -1
...
function generateRandonMineCoordinates(
  ...
  const getRandomMineCoor = (): Coordinate => {
    ...
    // no adjancent mines of seed coordinate unless small board
    const minDistance = height > 3 && width > 3 ? 2 : 1
    if (findCoordinateDistance(seedCoordinate, randCoor) < minDistance) {
  ...
  while (randomCoordinates.length !== numMines) {
    ...
    const count = randomCoordinates.filter((coor) =>
